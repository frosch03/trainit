...
exports.set = function (doc, req) {
     ...
     * CouchType takes care of generating a uuid
     */
    form.values._id = form.values.setnum + '-' + form.values.time; //req.uuid;

		if (form.isValid()) {
        return [form.values, {content: 'Set', title: 'Result'}];
    }
		else {
        ...
        return [null, {content : form.errors} ]; //{content: content, title: 'Set Form'}];
    }
};
